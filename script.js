...
async function initDashboard(isLanguageToggle = false) {
    try {
        ...
        if (!isLanguageToggle) {
            // Updating Hero Stats with fallbacks
            ...
            // Stats Bars - Using the data from JSON
            const s = data.stats || { first_serve_in: 64, break_points_saved: 73, first_return_won: 34, break_points_converted: 42 };
            document.getElementById('stat-serve-in').innerText = s.first_serve_in + '%';
            document.getElementById('bar-serve-in').style.width = s.first_serve_in + '%';
            document.getElementById('stat-bp-saved').innerText = s.break_points_saved + '%';
            document.getElementById('bar-bp-saved').style.width = s.break_points_saved + '%';
            document.getElementById('stat-ret-won').innerText = s.first_return_won + '%';
            document.getElementById('bar-ret-won').style.width = s.first_return_won + '%';
            document.getElementById('stat-bp-conv').innerText = s.break_points_converted + '%';
            document.getElementById('bar-bp-conv').style.width = s.break_points_converted + '%';
        ...
        // If everything fails, at least show the trophies and h2h fallbacks
        renderTrophies([]);
        renderH2H([]);
    }
}
